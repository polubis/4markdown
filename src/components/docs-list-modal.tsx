import { Button } from 'design-system/button';
import Modal from 'design-system/modal';
import React from 'react';
import { BiLowVision, BiRefresh, BiShow, BiWorld, BiX } from 'react-icons/bi';
import c from 'classnames';
import { differenceInDays, formatDistance } from 'date-fns';
import { Tabs } from 'design-system/tabs';
import type { API4MarkdownDto } from 'api-4markdown-contracts';
import { actReloadYourDocuments } from 'store/documents-creator/acts';
import { useDocumentsCreatorState } from 'store/documents-creator';
import { findActiveDocument } from 'store/documents-creator/selectors';
import { setActiveDocumentId } from 'store/documents-creator/actions';

type DocsListModalProps = {
  onClose(): void;
};

const rangeFilters = [`Recent`, `Old`, `Really Old`] as const;

type RangeFilter = (typeof rangeFilters)[number];

const rangeLookup: Record<RangeFilter, [number, number]> = {
  Recent: [0, 7],
  Old: [8, 30],
  'Really Old': [31, Number.MAX_VALUE],
};

const Content = ({ onClose }: DocsListModalProps) => {
  const busy = useDocumentsCreatorState((state) => state.busy);
  const activeDocument = useDocumentsCreatorState(findActiveDocument);
  const documents = useDocumentsCreatorState((state) => state.documents);
  const error = useDocumentsCreatorState((state) => state.error);

  const [activeRange, setActiveRange] = React.useState<RangeFilter>(
    rangeFilters[0],
  );

  const filteredDocuments = React.useMemo(() => {
    const now = new Date();

    return documents.filter((document) => {
      const diff = differenceInDays(now, document.mdate);
      const [from, to] = rangeLookup[activeRange];

      return diff >= from && diff <= to;
    });
  }, [documents, activeRange]);

  const selectDocument = (
    document: API4MarkdownDto<'getYourDocuments'>[number],
  ): void => {
    setActiveDocumentId(document.id);
    onClose();
  };

  if (busy) return <p className="text-2xl">Just a second (￣▽￣)...</p>;
  if (error)
    return (
      <p className="text-xl text-red-600 dark:text-red-400 text-center">
        Something went wrong... Try again with <strong>above button</strong>
      </p>
    );

  return (
    <>
      <Tabs className="mb-5">
        {rangeFilters.map((range) => (
          <Tabs.Item
            key={range}
            title={`${range} documents`}
            active={range === activeRange}
            onClick={() => setActiveRange(range)}
          >
            {range}
          </Tabs.Item>
        ))}
      </Tabs>
      {filteredDocuments.length > 0 ? (
        <ul className="flex flex-col space-y-3">
          {filteredDocuments.map((document) => (
            <li
              className={c(
                `flex flex-col cursor-pointer border-2 rounded-lg px-4 py-3`,
                activeDocument?.id === document.id
                  ? `bg-green-700 text-white border-green-700`
                  : `bg-zinc-200 dark:hover:bg-gray-900 dark:bg-gray-950 hover:bg-zinc-300 border-zinc-300 dark:border-zinc-800`,
              )}
              title={document.name}
              key={document.id}
              onClick={() => selectDocument(document)}
            >
              <div className="flex justify-between mb-0.5">
                <span className="text-sm capitalize">
                  Edited{` `}
                  {formatDistance(new Date(), document.mdate, {
                    addSuffix: true,
                  })}
                  {` `}
                  ago
                </span>
                {document.visibility === `private` && (
                  <BiLowVision size="20" title="This document is private" />
                )}
                {document.visibility === `public` && (
                  <BiShow size="20" title="This document is public" />
                )}
                {document.visibility === `permanent` && (
                  <BiWorld size="20" title="This document is permanent" />
                )}
              </div>
              <strong className="text-md">{document.name}</strong>
            </li>
          ))}
        </ul>
      ) : (
        <h6 className="p-4 text-center">No documents for selected filters</h6>
      )}
    </>
  );
};

const DocsListModal = ({ onClose }: DocsListModalProps) => {
  const busy = useDocumentsCreatorState((state) => state.busy);

  const close = (): void => {
    if (busy) return;
    onClose?.();
  };

  return (
    <Modal onEscape={close}>
      <div className="flex items-center justify-between gap-4 mb-6">
        <h6 className="text-xl">Your Documents</h6>
        <div className="flex gap-2">
          <Button
            type="button"
            i={2}
            s={1}
            title="Sync documents"
            disabled={busy}
            onClick={actReloadYourDocuments}
          >
            <BiRefresh />
          </Button>
          <Button
            type="button"
            i={2}
            s={1}
            disabled={busy}
            title="Close your documents"
            onClick={close}
          >
            <BiX />
          </Button>
        </div>
      </div>
      <Content onClose={onClose} />
    </Modal>
  );
};

export default DocsListModal;
